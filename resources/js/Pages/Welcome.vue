    ...
    <div class="h-full w-full fixed top-0 bg-gray-400">
      <div class="w-full h-full p-16">
        <div class="w-full h-full pb-16 px-48">
          <img class="h-full w-full shadow-2xl select-none rounded-lg" :src="assets.wallpaper" alt="daskom logo">
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    const elements = document.querySelectorAll('.daskom-text');
    elements.forEach((element) => {
      const text = element.textContent ?? '';
      element.innerHTML = text.replace(/([^\x00-\x80]|\w)/g, "<span class='daskom-text-letter inline-block'>$&</span>");
    });
